      ...
      {[...Array(totalSteps)].map((_, index) => (
        <React.Fragment key={index}>
          <div className="flex flex-col items-center">
            ...
            <span className={`text-xs mt-1 font-medium ${currentStep === index + 1 ? 'text-primary' : 'text-muted-foreground'}`}>
              {index === 0 ? 'Data Diri' : 
               index === 1 ? 'Detail Sewa' : 
               'Konfirmasi'}
            </span>
          ...
          {index < totalSteps - 1 && (
            <div className={`h-1 flex-1 mx-2 rounded ${currentStep > index + 1 ? 'bg-primary' : 'bg-muted'}`} />
          )}
        </React.Fragment>
      ))}
    </div>
