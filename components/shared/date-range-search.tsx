...
export default function DateRangeSearch({
  ...
  return (
    <Card className={`${cardClassName || "bg-card border-border"} shadow-md rounded-lg overflow-hidden`}>
      {(title || description) && (
        <CardHeader className="pb-2 space-y-1 bg-muted/40">
          {title && <CardTitle className="flex items-center gap-2"><CalendarIcon className="h-5 w-5" /> {title}</CardTitle>}
