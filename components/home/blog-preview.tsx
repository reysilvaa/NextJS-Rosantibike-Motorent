...
import { ChevronRight, Calendar, ArrowRight, Clock, User } from "lucide-react"
...
export default function BlogPreview() {
  ...
  const filteredPosts =
    activeCategory === "all" ? enhancedPosts : enhancedPosts.filter((post) => post.kategori === activeCategory)
                  ...
                  <Card className="overflow-hidden border-primary/10 shadow-xl shadow-primary/5 hover:shadow-2xl hover:shadow-primary/10 transition-all duration-300">
                    <div className="grid grid-cols-1 md:grid-cols-2">
                      <div className="relative h-64 md:h-auto">
                        <Image
                          src={featuredPost.featuredImage || "/placeholder.svg?height=600&width=800"}
                      ...
                      <div className="p-6 md:p-8 flex flex-col justify-center">
                        ...
                        <h3 className="text-2xl md:text-3xl font-bold mb-4">{featuredPost.judul}</h3>
                        <p className="text-foreground/80 mb-6 line-clamp-3">{featuredPost.konten}</p>
                    ...
                    <Card className="bg-card/50 border-border overflow-hidden hover:border-primary/50 transition-all hover:shadow-lg hover:shadow-primary/10 h-full">
                      ...
                      <CardFooter className="px-5 py-4 border-t flex items-center justify-between">
                        <div className="flex items-center">
                          ...
                          <span className="text-sm">{post.author ? post.author.nama : "Admin"}</span>
                        </div>
                        <Button
                          variant="ghost"
                          size="sm"
