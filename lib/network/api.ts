...
import type { MotorcycleType, MotorcycleUnit } from '../types/motorcycle';
import type { Transaction } from '../types/transaction';
import { API_CONFIG, getAuthHeader } from './api-config';
import { errorInterceptor, responseInterceptor } from './api-interceptor';
...
export async function fetchMotorcycleTypes(
  search?: string,
  filters?: Record<string, any>
): Promise<MotorcycleType[]> {
...
export async function fetchMotorcycleUnitById(id: string): Promise<MotorcycleUnit> {
  try {
    ...
    const response = await apiRequest<ApiResponse<MotorcycleUnit>>(
      `${API_CONFIG.ENDPOINTS.UNIT_MOTOR}/${id}`
    );
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        console.log(
          `Filtered ${availableUnits.length} available units out of ${units.length} total units`
        );
        ...
        const result: MotorcycleUnit[] = availableUnits.map((unit: BackendUnitAvailability) => {
          ...
          return {
            ...
            status: unit.status,
            hargaSewa:
              typeof unit.hargaSewa === 'string' ? parseInt(unit.hargaSewa) : unit.hargaSewa,
            jenis: unit.jenisMotor
              ? {
                  ...
                  gambar: null,
                  slug: (unit.jenisMotor.merk + '-' + unit.jenisMotor.model)
                    .toLowerCase()
                    .replace(/\s+/g, '-'),
        ...
        const result = (availabilityResponse as MotorcycleUnit[]).filter(
          unit => unit.status === 'TERSEDIA'
        );
      ...
      console.warn(
        'Invalid response format from availability endpoint, using alternative endpoint'
      );
    } catch (error) {
    ...
    const alternativeResponse = await apiRequest<ApiResponse<MotorcycleUnit[]> | MotorcycleUnit[]>(
      API_CONFIG.ENDPOINTS.UNIT_MOTOR
    );
...
export async function fetchTransactionHistory(): Promise<Transaction[]> {
  const response = await apiRequest<ApiResponse<Transaction[]>>(
    API_CONFIG.ENDPOINTS.TRANSAKSI_HISTORY
  );
  return Array.isArray(response) ? response : response.data || [];
}
...
export async function fetchTransactionById(id: string): Promise<Transaction> {
  const response = await apiRequest<ApiResponse<Transaction>>(
    `${API_CONFIG.ENDPOINTS.TRANSAKSI}/${id}`
  );
  return response.data;
}
...
export async function completeTransaction(id: string): Promise<Transaction> {
  const response = await apiRequest<ApiResponse<Transaction>>(
    `${API_CONFIG.ENDPOINTS.TRANSAKSI}/${id}/selesai`,
    {
      method: 'POST',
    }
  );
...
export async function fetchUserTransactions(): Promise<Transaction[]> {
  const response = await apiRequest<ApiResponse<Transaction[]>>(
    API_CONFIG.ENDPOINTS.TRANSAKSI_USER
  );
  return Array.isArray(response) ? response : response.data || [];
}
...
export async function checkMotorcycleAvailability(data: {
  ...
  try {
    ...
    // Log untuk debugging
    console.log(
      `Checking availability for unit ${data.unitId} from ${tanggalMulai} to ${tanggalSelesai}`
    );

    // Gunakan endpoint availability yang sudah ada dengan request GET
    const response = await apiRequest<any>(
      `${API_CONFIG.ENDPOINTS.UNIT_MOTOR}/availability?${params.toString()}`
    );
    ...
    if (response && response.units && Array.isArray(response.units)) {
      ...
      if (unit.availability && Array.isArray(unit.availability)) {
        ...
        console.log(
          `Unit availability check result: ${allDaysAvailable ? 'Available' : 'Not Available'}`
        );
...
export async function fetchAllMotorcycles(): Promise<MotorcycleType[]> {
  try {
    ...
    if (missingSlug.length > 0) {
      console.warn(
        `Warning: ${missingSlug.length} motorcycles have missing slugs:`,
...
export async function fetchAllBlogPosts(): Promise<BlogPost[]> {
  try {
    ...
    if (missingSlug.length > 0) {
      console.warn(
        `Warning: ${missingSlug.length} blog posts have missing slugs:`,
