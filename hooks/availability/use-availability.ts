...
import {
  useAvailability as useMotorcycleAvailability,
  useMotorcycleTypes,
} from '@/hooks/use-motorcycles';
...
export function useAvailability() {
  ...
  const {
    data: availableMotorcycles,
    isLoading,
    error,
  } = useMotorcycleAvailability(searchParamsObj);
  ...
  const rentalDays =
    canSearch && tanggalMulai && tanggalSelesai
      ? differenceInDays(
          parse(tanggalSelesai, 'yyyy-MM-dd', new Date()),
          parse(tanggalMulai, 'yyyy-MM-dd', new Date())
        )
      : 0;

  // Sort motorcycles
  const sortedMotorcycles = availableMotorcycles
    ? [...availableMotorcycles].sort((a, b) => {
        switch (sortOption) {
          case 'price-asc':
          ...
          case 'name-asc':
            return `${a.jenis?.merk || ''} ${a.jenis?.model || ''}`.localeCompare(
              `${b.jenis?.merk || ''} ${b.jenis?.model || ''}`
            );
          case 'name-desc':
            return `${b.jenis?.merk || ''} ${b.jenis?.model || ''}`.localeCompare(
              `${a.jenis?.merk || ''} ${a.jenis?.model || ''}`
            );
          default:
            return 0;
        }
