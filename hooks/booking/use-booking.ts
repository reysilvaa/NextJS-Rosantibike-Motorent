...
export function useBooking({ motorcycle, startDate, endDate, onSuccess }: UseBookingParams) {
  ...
  const { isLoading: isCheckingAvailability, withLoading: withAvailabilityLoading } = useLoading(false);
  ...
  const checkAvailability = useCallback(async () => {
    try {
      const response = await withAvailabilityLoading(checkMotorcycleAvailability({
      ...
      if (!isMotorcycleAvailable) {
        setFormError("Motor tidak tersedia pada rentang waktu yang dipilih. Silakan pilih waktu lain.");
        return false;
      }
  ...
  }, [motorcycle.id, startDate, endDate, formData.jamMulai, formData.jamSelesai, withAvailabilityLoading]);
  ...
  const fetchPriceCalculation = useCallback(async () => {
    try {
      const result = await withCalculatingLoading(apiCalculateRentalPrice({
        unitId: motorcycle.id,
        tanggalMulai: format(startDate, "yyyy-MM-dd"),
        tanggalSelesai: format(endDate, "yyyy-MM-dd"),
        jamMulai: formData.jamMulai,
  ...
  }, [motorcycle.id, startDate, endDate, formData.jamMulai, formData.jamSelesai, formData.jasHujan, formData.helm, toast, withCalculatingLoading]);
  ...
  const handleSubmit = useCallback(async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const result = await withLoading(createTransaction({
  ...
  }, [backendPriceDetails, priceBreakdown.totalPrice, withLoading, formData, motorcycle, startDate, endDate, toast, onSuccess, router, checkAvailability]);
